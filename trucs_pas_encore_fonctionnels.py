...
monde_test = Monde(8, 20)
        

class Poisson(Monde):
    
    def __init__(self, x, y, fish="🐠"):
        super().__init__(self.nb_lignes, self.nb_colonnes)  # Appelez le constructeur de la classe parente avec les arguments x et y

        # Dimensions de la grille
        # self.nb_lignes = nb_ligne
        # self.nb_colonnes = nb_colonnes
    ...
    def deplacements(self):
        ...
        indices_adjacents = [((poisson.x-1)% Monde.nb_colonnes, (poisson.y)% Monde.nb_lignes), ((poisson.x+1)%Monde.nb_colonnes, (poisson.y)% Monde.nb_lignes),
                                ((poisson.x)%Monde.nb_colonnes, (poisson.y-1)%Monde.nb_lignes), ((poisson.x)%Monde.nb_colonnes, (poisson.y+1)%Monde.nb_lignes)]
